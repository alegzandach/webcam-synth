...
(function (root, factory) {
    ...
    var SuperGif = function ( opts ) {
        ...
        var pushFrame = function () {
            ...
            const colors = getColors(data.data)
            //console.log(colors);
            const sections = divyUp(colors, 3, 4, hdr.width, hdr.height);
            //console.log(sections);
            const voices = create2DArray([3, 4]);
            for(let i=0;i<3;i++){
                for(let j=0;j<4;j++){
                    voices[i][j] = average(sections[i][j])
                }   
            }
            //console.log(voices);
            frames.push({
